...
--
```{r comment=NA}
mean(df$hwy)  # Average value for mpg
```
--
```{r comment=NA}
# Need to pay attention to additional arguments
mean(df$hwy, na.rm=TRUE)
```
 ...
 * What if we wanted to add more data (i.e., row)?
 
```{r comment=NA}
# Let's add a new row of data with the average for each column
newrow <- df[1,]
```
 ...
 * Helpful functions - **dplyr**
  ...
  + *filter()* picks cases based on their values.
```{r comment=NA, echo=FALSE, error=FALSE, message=FALSE}
library(dplyr)
library(ggplot2)
```

```{r comment=NA}
ford <- mpg %>% filter(hwy>=25)
head(ford, 5)
```
 ...
 * Helpful functions - **dplyr**
  ...
  + *select()* picks cases based on their values.
```{r comment=NA}
hwy <- mpg %>% select(hwy)
head(hwy, 5)
```
 ...
 * *geom_()* function - adds a layer. For example, *geom_point* creates a scatterplot. 
