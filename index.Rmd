 ...
 * [**Topic 2:**](<https://jeremymack-lu.github.io/rprog/#23>) What is RStudio? Why use it?
 
 * [**Topic 3:**](<https://jeremymack-lu.github.io/rprog/#35>) Getting started with R and RStudio
 
 * [**Topic 4:**](<https://jeremymack-lu.github.io/rprog/#61>) Objects and Functions
 
 * [**Topic 5:**](<https://jeremymack-lu.github.io/rprog/#101>) Data and Data Wrangling
 
 * [**Topic 6:**](<https://jeremymack-lu.github.io/rprog/#182>) Tidyverse
 
 * [**Topic 7:**](<https://jeremymack-lu.github.io/rprog/#205>) Other things to do with R

---
#### Programming in R

<center><img src="./images/r_rollercoaster.png" alt="RStudio" height=350/></center>
 ...
 *Collectively referred to as Numeric
...
typeof(z)               # Print the class name of z
```
]
...
* Standard functions are a part of **base R**, which loads automatically when R is opened. There is also a large community of users that develop packages for R (17,400+ and counting!).

--

* Packages can be loaded both programmatically and with the RStuio explorer window.
 ...
 * ~~Check and/or set a working directory.~~ Set up an RStudio project.
...
* What if we read the data in as a tibble?
...
.tiny6[
```{r, comment=NA, eval=FALSE}
# Read table into R as a data frame
mpg <- read.csv("data/mpg.csv"); str(mpg)
```
...
.tiny6[
```{r, comment=NA, eval=FALSE}
# Read table into R as a tibble
mpg2 <- read_csv("data/mpg.csv"); glimpse(mpg2)
```
 ...
 * Data can be read into R both programmatically and with the RStudio IDE.

--

 * RStudio projects provide structure to data projects that enhances reproducibility.
 ...
 * So far, we've mostly used base R commands and functions to wrangle data.
 ...
 * Let's load the core Tidyverse packages:
...
# Install and load tidyverse packages (pacman package)
 ...
 * **dplyr** package
  ...
  + Introduces consistent set of functions (verbs) used for data wrangling.
     ]
...
```{r comment=NA}
# base R approach:
hwy.avg <- mean(mpg$hwy)
hwy.max <- max(mpg$hwy)
hwy.min <- min(mpg$hwy)
hwy.sd  <- sd(mpg$hwy)
do.call("cbind",
        list(hwy.avg,hwy.max,hwy.min,hwy.sd))
```
...
# tidyverse approach with piping:
mpg <- mpg %>% mutate(diff=cty-hwy)
```
]

.tiny6[
```{r comment=NA, echo=FALSE}
head(data.frame(mpg),5)
```
]
...
* References for R and working with the Tidyverse:
